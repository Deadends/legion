...
impl RocksDBMerkleTree {
    pub fn new(db_path: &str) -> Result<Self> {
        ...
        let next_index = db.get(b"meta:next_index")?
            .and_then(|bytes| {
                if bytes.len() == 8 {
                    let mut arr = [0u8; 8];
    ...
    pub fn get_proof(&self, index: usize) -> Result<([Fp; MERKLE_DEPTH], Fp)> {
        ...
        
        {
            ...
            for level in 0..MERKLE_DEPTH {
                ...
                path[level] = db.get(key.as_bytes())?
                    .and_then(|bytes| {
                        if bytes.len() == 32 {
                            let mut arr = [0u8; 32];
                            arr.copy_from_slice(&bytes);
    ...
    fn update_path_batch(&self, db: &DB, batch: &mut WriteBatch, leaf_index: usize, leaf: Fp) -> Result<()> {
        ...
        for level in 0..MERKLE_DEPTH {
            ...
            let sibling_value = db.get(sibling_key.as_bytes())?
                .and_then(|bytes| {
                    if bytes.len() == 32 {
                        let mut arr = [0u8; 32];
                        arr.copy_from_slice(&bytes);
            ...
            current_value = poseidon::Hash::<_, poseidon::P128Pow5T3, poseidon::ConstantLength<2>, WIDTH, RATE>::init()
                .hash([left, right]);
