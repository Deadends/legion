...
use ml_kem::{MlKem768, KemCore, Encapsulate, Decapsulate};
#[cfg(feature = "post-quantum")]
...
#[cfg(not(feature = "post-quantum"))]
pub fn generate_hybrid_keys() -> HybridKeyState {
    HybridKeyState { dummy: vec![0u8; 32] }
}
...
pub fn process_hybrid_payload(peer_payload: &[u8]) -> Result<(Vec<u8>, Vec<u8>)> {
    ...
    let (ml_kem_ct, ml_kem_ss) = peer_ml_kem_ek.encapsulate(&mut OsRng)
...
#[cfg(feature = "post-quantum")]
pub fn finalize_hybrid_exchange(
    state: &HybridKeyState,
    peer_response: &[u8],
) -> Result<Vec<u8>> {
    ...
    let ml_kem_ss = state.ml_kem_dk.decapsulate(&ml_kem_ct)
