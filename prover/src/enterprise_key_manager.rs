...
impl EnterpriseKeyManager {
    ...
    pub fn get_server_key() -> Result<[u8; 32]> {
        Self::try_encrypted_storage_key().or_else(|_| Self::derive_deterministic_key())
    }
    ...
    fn encrypt_server_key(key: &[u8; 32], master_key: &[u8; 32]) -> Result<Vec<u8>> {
        ...
        let tag = cipher
            .encrypt_in_place_detached(nonce, b"LEGION_SERVER_KEY", &mut buffer)
    ...
    fn decrypt_server_key(encrypted: &[u8], master_key: &[u8; 32]) -> Result<[u8; 32]> {
        ...
        cipher
            .decrypt_in_place_detached(nonce, b"LEGION_SERVER_KEY", &mut buffer, tag.into())
