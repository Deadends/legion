...
use halo2_gadgets::poseidon::{primitives as poseidon, Hash as PoseidonHash, Pow5Chip, Pow5Config};
use halo2_proofs::{
...
impl AuthCircuit {
    ...
    pub fn public_inputs(&self) -> Vec<Fp> {
        ...
        let pubkey_binding = poseidon::Hash::<
            _,
            poseidon::P128Pow5T3,
            poseidon::ConstantLength<2>,
            WIDTH,
    ...
    pub fn public_inputs_for_verifier(&self, auth_context: &crate::AuthContext) -> Vec<Fp> {
        vec![
            ...
            Fp::from_uniform_bytes(
                &[
                    auth_context.challenge_hash[..32].try_into().unwrap(),
                    [0u8; 32],
                ]
                .concat()
                .try_into()
            ...
            Fp::from_uniform_bytes(&{
                let mut buf = [0u8; 64];
                buf[..16].copy_from_slice(&auth_context.session_id);
                buf
            }),
            Fp::from(1u64),
    ...
    #[allow(dead_code)]
    fn verify_merkle_path(leaf: Fp, path: &[Fp; MERKLE_DEPTH], root: Fp, index: u64) -> bool {
...
impl Circuit<Fp> for AuthCircuit {
    ...
    fn configure(meta: &mut ConstraintSystem<Fp>) -> Self::Config {
        ...
        let poseidon_config =
            Pow5Chip::configure::<poseidon::P128Pow5T3>(meta, state, partial_sbox, rc_a, rc_b);
    ...
    fn synthesize(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                region.assign_advice(
                    ...
                    config.advice[0],
                    0,
                    || computed_credential_hash.value().copied(),
                )?;
                region.assign_advice(
                    || "stored",
                    config.advice[1],
                ...
                |mut region| {
                    ...
                    let left = region.assign_advice(
                        ...
                        || {
                            direction_bit
                                .value()
                                .zip(current_hash.value())
                                .zip(sibling_cell.value())
                                .map(|((bit, curr), sib)| {
                                    // left = bit * sibling + (1 - bit) * current
        ...
        let pubkey_binding_hasher = PoseidonHash::<
            _,
            _,
            poseidon::P128Pow5T3,
            poseidon::ConstantLength<2>,
        ...
        for level in 0..DEVICE_TREE_DEPTH {
            ...
            let (left_input, right_input) = layouter.assign_region(
                ...
                |mut region| {
                    ...
                    region.assign_advice(
                        ...
                        config.advice[1],
                        0,
                        || current_device_hash.value().copied(),
                    )?;
                    region.assign_advice(
                        || "sibling",
                        config.advice[2],
                    ...
                    let left = region.assign_advice(
                        ...
                        || {
                            direction_bit
                                .value()
                                .zip(current_device_hash.value())
                                .zip(sibling_cell.value())
                                .map(|((bit, curr), sib)| *bit * sib + (Fp::one() - bit) * curr)
                        },
                    )?;
                    ...
                    let right = region.assign_advice(
                        ...
                        || {
                            direction_bit
                                ...
                                .zip(current_device_hash.value())
                                .zip(sibling_cell.value())
                                .map(|((bit, curr), sib)| *bit * curr + (Fp::one() - bit) * sib)
                        },
                    )?;
            ...
            let device_hasher = PoseidonHash::<
                _,
                _,
                poseidon::P128Pow5T3,
                poseidon::ConstantLength<2>,
        ...
        let session_token_hasher = PoseidonHash::<
            _,
            _,
            poseidon::P128Pow5T3,
        ...
        let session_token = session_token_hasher.hash(
            ...
            [
                computed_nullifier.clone(),
                timestamp_cell.clone(),
                linkability_tag_cell.clone(),
            ],
        )?;
