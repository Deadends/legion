...
use crate::{
    ...
    fill_random_bytes, get_timestamp,
...
impl AuthenticationProtocol {
    pub fn new() -> Result<Self> {
        ...
        let data_dir =
            std::env::var("LEGION_DATA_PATH").unwrap_or_else(|_| "./legion_data".to_string());
        ...
        let anonymity_tree = Arc::new(RwLock::new(AnonymityMerkleTree::new_with_rocksdb(
            &rocksdb_path,
        )?));
    ...
    pub fn get_anonymity_set_dto(&self) -> Result<AnonymitySetData> {
        ...
        let leaves_hex: Vec<String> = tree
            .get_leaves()
            .iter()
    ...
    pub fn register_device(
        ...
        let (position, root) = self
            .device_tree_manager
            .register_device(nullifier_hash, device_commitment)?;
        Ok((position, hex::encode(root.to_repr())))
    }

    /// Get device proof for user
    pub fn get_device_proof(
        &self,
        nullifier_hash: &str,
        position: usize,
    ) -> Result<(Vec<String>, String)> {
        let (path, root) = self
            .device_tree_manager
            .get_device_proof(nullifier_hash, position)?;
    ...
    pub fn verify_anonymous_proof(
        ...
        let expiration_u64 =
            u64::from_le_bytes(expiration_bytes[..8].try_into().unwrap_or([0u8; 8]));
    ...
    pub fn verify_proof(
        ...
        if public_inputs.len() != 10 {
            return Err(anyhow!(
                "Invalid public inputs length: expected 10, got {}",
                public_inputs.len()
            ));
        }
        ...
        Self::new().unwrap_or_else(|_| {
            ...
            #[cfg(feature = "redis")]
            {
                Self {
                    ...
                    device_tree_manager,
                    redis_cache: Arc::new(RwLock::new(
                        crate::redis_cache::RedisCache::new().unwrap(),
                    )),
                    bloom_filter: Arc::new(RwLock::new(
                        crate::bloom_filter::BloomFilter::new().unwrap(),
                    )),
                }
            }
