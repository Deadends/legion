...
mod nova_impl {
    ...
    impl StepCircuit<Fp> for MerklePathStepCircuit {
        ...
        fn synthesize<CS: ConstraintSystem<Fp>>(
            ...
            let sibling =
                AllocatedNum::alloc(cs.namespace(|| "sibling"), || Ok(self.sibling_node))?;
            ...
            let mut sponge =
                Sponge::new_with_constants(&constants, neptune::sponge::vanilla::Mode::Simplex);
            let mut acc = ();
            sponge
                .absorb(&left_val, &mut acc)
                .map_err(|_| SynthesisError::Unsatisfiable)?;
            sponge
                .absorb(&right_val, &mut acc)
                .map_err(|_| SynthesisError::Unsatisfiable)?;
            let parent_val = sponge
                .squeeze(&mut acc)
                .map_err(|_| SynthesisError::Unsatisfiable)?
                .ok_or(SynthesisError::AssignmentMissing)?;
    ...
    pub fn generate_nova_proof(
        ...
        println!(
            "🚀 Generating Nova Merkle proof ({} steps)...",
            MERKLE_DEPTH
        );
        ...
        println!("  - Verifying...");
        let (z_out_primary, _) =
            recursive_snark.verify(&pp, MERKLE_DEPTH, &z0_primary, &z0_secondary)?;
