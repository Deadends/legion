...
impl KeyRotationManager {
    ...
    pub fn add_credential(
        ...
        let leaf_hash = self.compute_versioned_leaf(
            username_hash,
            password_hash,
            version,
            expires_at,
    ...
    pub fn rotate_password(
        ...
        let next_version = self.credentials
            .get(&username_key)
        ...
        // Add new version
        let result = self.add_credential(username_hash, new_password_hash, next_version, expiry_days)?;
        self.save_to_storage()?;
        Ok(result)
    }
    ...
    pub fn get_active_version(&self, username_hash: &Fp) -> Option<&CredentialVersion> {
        ...
        self.credentials.get(&username_key)?.iter()
    ...
    fn compute_versioned_leaf(
        ...
        let combined = poseidon::Hash::<Fp, poseidon::P128Pow5T3, poseidon::ConstantLength<2>, 3, 2>::init()
            .hash([*username_hash, *password_hash]);
        let versioned = poseidon::Hash::<Fp, poseidon::P128Pow5T3, poseidon::ConstantLength<2>, 3, 2>::init()
            .hash([combined, version_fp]);
        Ok(poseidon::Hash::<Fp, poseidon::P128Pow5T3, poseidon::ConstantLength<2>, 3, 2>::init()
...
mod tests {
    ...
    fn test_key_rotation() {
        ...
        let leaf1 = manager.add_credential(&username, &password1, 0, 90).unwrap();
