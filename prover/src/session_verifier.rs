...
impl SessionVerifier {
    ...
    pub fn create_session(client_public_key: Vec<u8>, nullifier: [u8; 32]) -> Result<String> {
    ...
    pub fn verify_request(session_id: &str, request_data: &[u8], signature: &[u8]) -> Result<bool> {
        let mut store = SESSION_STORE.write().unwrap();
        ...
        // Verify signature
        let expected_sig =
            Self::compute_signature(session_id, &session.client_public_key, request_data)?;
